...
The number of observations available for Version 2 of the dependent variable in this dataset is smaller than the number of parameters in the model, rendering the model impossible to estimate without incorporating additional information (for example, via priors in a Bayesian approach).  
...
Our approach is otherwise identical to that of the core analysis of NJL: Table 1, Model 1.
...
Income segregation has no discernible effect on meritocratic beliefs in the results for Model 3A.  
...
<<label=b_cz_data_merge>>==
# Merge Data
load("mdf_mi_list.rda") # Individual-Level Data: Pew 2007 RLS, multiply imputed and saved in merit.Rnw

mdf_mi_list <- mdf_mi_list[[1]] %>% 
  map(. %>% 
        left_join(cz, by="fips") %>% 
        left_join(cz_all, by="cz"))

mdf_mi_list <- imputationList(mdf_mi_list)
@

<<label=b_m1_cz, results="asis">>==
# Inequality by income (replication of main text results)
m1_cz <- suppressMessages(
  with(mdf_mi_list, 
       glmer(formula = rej_merit ~ gini_cz*income + 
...
m2_cz <- suppressMessages(
  with(mdf_mi_list,
       glmer(formula = rej_merit ~ gini_cz*income + rm_cz +
               educ + age + male + noncitizen +
               latino + black + asian + other_min +
               partyid_rep + ideo_con + attend +
               income_cz + black_cz + bush04_cz + pop_cz +
...
format_mi_results <- function(m, mod) {
...
m123_cz_df <- rbind(format_mi_results(m1_cz, "Model 1A"),
                 format_mi_results(m2_cz, "Model 2A"),
                 format_mi_results(m3_cz, "Model 3A")) %>%
  by_2sd(mdf_mi_list[[1]][[1]])
...
<<label=b_m123_cz_gini_plot, cache=FALSE>>=
