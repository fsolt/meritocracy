...
Figure~\ref{F:coef_plot} plots the conditional effect of inequality at each observed value of income.  
...
Of course, given the dichotomous dependent variable, these estimates are in logits and so their magnitudes are not easily interpretable.  
