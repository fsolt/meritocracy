...
p2007x <- merge(p2007, p2007fips) %>% 
  transmute(resp = psraid,
            ...
            other_race = ifelse(race<=4, (race==4)+0, NA),
...
m0 <- glmer(formula = merit ~ gini_cz_c +
              income_c + educ + age + male +
...
m1 <- glmer(formula = merit ~ gini_cz_c + income_c:gini_cz_c +
              income_c + educ + age + male +
              partyid_rep + ideo_con + attend +
              income_cz + black_cz + bush04_cz + pop_cz +
              (1+income|cz),
...
p_m1 <- interplot(m1, "gini_cz", "income") +
...
ggsave(p_m1, "figures/m1.pdf")

# Inequality by Income (With Relative Mobility)
m2 <- glmer(formula = merit ~ rm_cz + gini_cz_c + income_c:gini_cz_c +
              income_c + educ + age + male +
              partyid_rep + ideo_con + attend +
              income_cz + black_cz + bush04_cz + pop_cz +
              (1+income|cz),
...
p_m2 <- interplot(m2, "gini_cz_c", "income_c") +
  scale_x_discrete(limits = inc_labels) + 
  labs(x = "Family Income", y = "Local Income Inequality") +
  theme_bw() +
  geom_hline(yintercept=0, colour="grey80", linetype="dashed")
ggsave("figures/m2.pdf", p_m2)
...
ordered_vars <- c("rm_cz", "gini_cz_c", "gini_cz_c:income_c", "income_c",
                  "educ", "age", "male",
                  "partyid_rep", "ideo_con", "attend",
                  "income_cz", "black_cz", "bush04_cz", "pop_cz")

dwplot(m123_df, order_vars = ordered_vars) +
     relabel_y_axis(c("Relative Mobility", 
                      "Income Inequality", "Income Inequality x Income",
                      "Income", "Education", "Age", "Male",
     ...
     ggtitle("Predicting Belief in Meritocracy") +
