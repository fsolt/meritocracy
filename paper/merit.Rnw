...
We demonstrate here, however, that \citet{Newman2015} crucially misinterprets the interaction term in its model \citep[see, e.g.,][]{Brambor2006}.  Correcting this error reveals that there is no support for the paper's sanguine conclusion that mere exposure to high levels of inequality stimulates a rejection of meritocracy. 

Further, in reproducing the above result from the materials the authors provided \citep{Newman2015a}, we uncovered other problems.  
...
\citet{Newman2015} advocate the conflict theory.  
...
Their greater relative powerlessness leads to ``a greater susceptibility to the internalization of the values, beliefs, or rules of the game of the powerful as a further adaptive response---i.e., as a means of escaping the subjective sense of powerlessness, if not its objective condition'' \citep[17]{Gaventa1980}.
...
\section*{Reproduction, Results, and Reinterpretation}

It has been well known for over a decade that models containing multiplicative interaction terms require particular care in interpretation \citep[see, e.g.,][]{Golder2003, Braumoeller2004, Brambor2006}. 
Nevertheless, many political scientists struggle with interaction terms: improperly specified or interpreted interaction terms appear at the top of Nyhan's (\citeyear{Nyhan2015}) list of ``recurring statistical errors'' that reviewers should be sure to check for.
...
In the multilevel logistic regression model employed in \citet{Newman2015}, the logged odds of rejecting meritocracy for individual $i$ in local context $j$ can be expressed as: 
\end{spacing}
\vspace{-36pt}
...
\vspace{-24pt}
\begin{spacing}{2}
Oddly, in \citet{Newman2015}, the nine categories of income in the Pew surveys are recoded to take on nine evenly-spaced values ranging from .21 to 1 \citep[see][]{Newman2015a}.
...
Because much redistribution occurs through the tax code, an after-tax measure would be preferable; unfortunately, virtually no data on the distribution of income at any geographic scale below the national level is available for the United States \citet[see, e.g.,][420]{Kelly2012}.  
...
Those who chose the first statement were coded 0 and others coded 1, yielding a dichotomous variable of belief in meritocracy.
\footnote{This is Version 1 of the three measures employed by \citet{Newman2015}.}
In these data, 33\% of respondents rejected meritocracy.  
The entire sample is analyzed, rather than only the subset of white non-Latino respondents, so as to make use of all of the available data, although \citet[330]{Newman2015} note that this should be expected to bias the results toward the expectations of the conflict theory.
