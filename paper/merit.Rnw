...
Correcting this error reveals that there is little or no support for the paper's conclusion that mere exposure to high levels of inequality stimulates a rejection of meritocracy. 

Further, in reproducing the above result from the materials the authors provide \citep{Newman2015a}, we uncovered problems with the manner in which the dependent variable was measured. 
We therefore present here a full, independent replication, bringing data from a larger and more representive survey that employs a uniform measure of the dependent variable to examine how, if at all, local contexts across the United States shape beliefs about whether people can get ahead if they are willing to work hard.  
