...
For the insistent, however, models including these variables are presented here in Figure~\ref{F:app_reg_plot} and Figure~\ref{F:app_coef_plot}.
...
The results lend no support to cognitive hypothesis suggested above: it is those living in context of \emph{greater} relative mobility who are more likely to reject meritocracy, and the results regarding the effect of income inequality are essentially unchanged.
