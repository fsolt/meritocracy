...
ggsave(file="paper/figures/interaction_njl_t1m1.pdf", plot=t1m1_plot, width=8, height=5.25)
@

\begin{figure}[htbp] 
  \caption{Logit Coefficients of Local Income Inequality on Belief in Meritocracy by Respondent Income: Table 1, Model 1, From Replication Data}
  \label{F:coef.t1m1}
  \begin{center}
  ...
  & \emph{Notes}: The dependent variable is coded so that 1 indicates belief in meritocracy, not rejection of meritocracy as in \citet{Newman2015}. The coefficient for county income inequality fails to reach statistical significance for any observed level of respondent family income.
  \end{tabular}
  \end{footnotesize}
\end{figure}
...
ver_all <- bind_rows(ver_a1, ver_a2, ver_b, ver_c) %>% 
  select(one_of("year", "version", "value", "se"))
...
ggplot(ver_all2, aes(x = year, y = value, color = version)) +
  ...
  geom_linerange(aes(ymin=value - qnorm(.975) * se, ymax=value + qnorm(.975) * se), position = position_dodge(width = 0.08)) +
	geom_line(position = position_dodge(width = 0.08), alpha = .5, linetype = 3) +
...
ggsave("./paper/figures/three_measures.pdf", width = 4, height = 6)
@
...
\begin{figure}[!hbtp] 
  \caption{Predicting Belief in Meritocracy With Additional Controls}
