...
However, due to misinterpretation of the interaction term in its model \citep[see, e.g.,][]{Brambor2006}, the article's empirical results do not support its conclusion, and efforts to replicate its analyses revealed a number of additional severe problems, from unjustifiably pooling incomparable observations to errors in merging together contextual and survey data to neglecting available data that does not support the hypothesis \citep{Solt2016a}.  That work's sanguine conclusion, that mere exposure to high levels of inequality stimulates a rejection of meritocracy (and so should generate increased demand for redistribution) among those with lower incomes, then, cannot be accepted without further investigation. 


Activated class conflict theory
