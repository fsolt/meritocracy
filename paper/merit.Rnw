...
\title{Economic Inequality and\\ Belief in Meritocracy in the United States}
\date{}				
\maketitle
...
This analysis finds no evidence for the argument of NJL (p.329) that high levels of economic inequality work to activate an ``oppositional consciousness'' among lower-income individuals and so are ultimately self-correcting.  
...
The models are estimated using multilevel logistic regression of individuals nested in counties, with both the intercept and the coefficient for income allowed to vary across counties. 
...
For people with incomes between \$40,000 to \$50,000 and otherwise median characteristics, the predicted probability of rejecting meritocracy declines from $40\pm 3$\% to $32\pm 4$\% over the observed range of inequality.
