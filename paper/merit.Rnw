...
At the other end of the income scale, there is no hint of support for the claim that these results support the conflict theory's prediction that \emph{higher}-income individuals will be \emph{less} likely to reject meritocracy in contexts of greater income inequality.
...
In a footnote and the article's online appendix, however, NJL argues that mixing these different measures is not problematic on the basis that the 2005 and 2006 surveys alone, which use Measure 1, yield results for the variables of interest similar to those produced by the four surveys together (NJL, p.331).  
...
m1_pp_plot <- ggplot(m1_pp, aes(x = gini_cnty, y = est, group = income)) +
  ...
  geom_text(aes(.36, 55, label="<$10k", color="1")) +
  geom_text(aes(.4, 37.5, label="$40-50k", color="5")) +
