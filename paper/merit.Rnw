...
p2007x1_w <- p2007x1 %>%
  filter(white==1) %>% 
  select(-white) %>% 
  filter(!is.na(rm_cz)) # 24 obs from 16 very small CZ without mobility data
@

<<label=impute, echo=F, results='hide'>>==
# Multiply impute missing values with mi
p1x_w_mi <- missing_data.frame(p2007x1_w)
...
m3_mi <- with(p1x_w_mi_list2, 
                glmer(formula = rej_merit ~ rm_cz + gini_cz +
                        income + educ + age + male + 
                        partyid_rep + ideo_con + attend +
                        income_cz + black_cz + bush04_cz + pop_cz +
                        (1+income|cz),
                      family=binomial(link="logit")))
m3_mi_fe <- MIextract(m3_mi, fun=fixef) # https://books_google_com/books?id=EbLrQrBGid8C&pg=PA384
m3_mi_vars <- MIextract(m3_mi, fun=vcov)
m3_mi_vars2 <- list()
m3_mi_vars2 <- lapply(m3_mi_vars, as.matrix)
m3_mi_res <- MIcombine(m3_mi_fe, m3_mi_vars2)
summary(m3_mi_res)
