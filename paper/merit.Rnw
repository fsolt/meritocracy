...
One prominent recent study, \citet{Newman2015}, argues that exposure to higher levels of local income inequality lead low-income people to become more likely to reject the dominant U.S. ideology of meritocracy.  
Here, we show that this sanguine conclusion is not supported by the study's own empirical results and further that analysis of new data from a larger and more representative survey that employs a single consistent measure of the dependent variable yields precisely the opposite conclusion.  
Consistent with the relative power theory, among those with lower incomes, local contexts of greater inequality are associated with more widespread belief that people can get ahead if they are willing to work hard.   %136 words
\end{abstract}
...
Further, in reproducing the NJL analyses from the materials the authors provide \citep{Newman2015a}, we uncovered problems with the manner in which the dependent variable was measured that would render the article's results untrustworthy even if they had supported its conclusions. 
We therefore present here a full, independent replication, bringing new data from a larger and more representative survey that employs a single consistent measure of the dependent variable to examine how, if at all, local contexts across the United States shape beliefs about whether people can get ahead if they are willing to work hard.  
...
Using the data files and \R commands provided \citep{Newman2015a}, we were able to reproduce a close approximation of the article's main results.\footnote{\doublespacing We were not able to reproduce the analysis on class consciousness presented in Table 3 at all, however, as the number of random effects to be estimated in the model, 1322, exceeds the number of observations in the data, 1067 (see NJL, p.336; see also \citealt{Solt2016c}).  
...
\section*{Replication}
In reproducing the above analysis, our attention was drawn to an additional issue that led us to pursue an independent replication: the measurement of the dependent variable.  
...
We also confirm that the conditional effect of inequality fails to reach statistical significance at any observed level of income, just as in the full sample described above, regardless of how the dependent variable is measured.
The results presented in NJL would appear to be an artifact generated by the authors' decision to pool these three very different measures together.
...
In our replication, we instead employ the U.S. Religious Landscape Survey (RLS) conducted by the Pew Forum on Religion and Public Life in 2007, which includes Measure 1 of the dependent variable.  
...
Unfortunately, the article's own reported results do not support this conclusion, and its analysis suffers from measurement issues that render even those results untrustworthy.
Our independent replication using data from a larger and more representative survey that employs a uniform measure of the dependent variable, in fact, yields the opposite result: lower-income people living where local levels of income inequality are higher are less likely to reject meritocracy than those living where the income distribution is more egalitarian, in line with the predictions of the relative power theory.
