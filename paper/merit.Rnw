...
\section*{Replication}
In reproducing the above analysis, our attention was drawn to two further serious issues that prompted us to pursue an independent replication.  
The first issue regards measurement of the dependent variable.  
...
The second serious issue with regards the contextual unit of analysis.  
\citet[331-332]{Newman2015} employs the respondent's county of residence for this purpose.  
Counties, however, have long been criticized as only poorly reflecting local economic and labor force conditions \citep[see, e.g.,][]{Tolbert1987}.
Instead, we use respondents' commuting zones (CZs) to define the local context.  CZs are aggregations of counties meant to represent the scope of local economic relationships both in metropolitan areas and across the rural United States.  
...
Of course, it is well understood that controlling for variables that are causally downstream from an independent variable ``messes up'' the estimates of that independent variable's effect on the dependent variable \citet[188]{Gelman2007}.\footnote{On the powerful relationship between rising income inequality and greater religiosity, see, for example, \citet{Solt2011a, Solt2014b}.}
