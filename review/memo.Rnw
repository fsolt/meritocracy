...
We relegated the analysis on commuting zones to Appendix~B, where they are joined by the further analyses of the potentially confounding effects of relative mobility and income segregation that we presented there in the original draft.
...
Although this view overlooks the fact there is no hint of support for the article's claim that its results support the conflict theory's prediction that higher-income individuals will be \emph{less} likely to reject meritocracy in contexts of greater income inequality, it is nevertheless one that some readers of our piece may share.

We certainly agree that the difference between \textit{p}-values just below and just above 0.05 should not be overstated.  As \citet[328]{Gelman2006} famously wrote, ``the difference between `significant' and `not significant' is not itself statistically significant.''  But even if the coefficient estimate for the very lowest income Americans were statistically significant, it would provide minimal support for the conflict theory.

We again direct attention to the substantial literature in political methodology regarding the correct use and interpretation of interaction terms. \citet{Berry2012} addresses this point directly.  In discussing the circumstance in which the estimated conditional effect of $X$ is statistically significant only for a range of values of the conditioning variable $Z$, \citet[660-661]{Berry2012} points out that the extent of support for the hypothesis that $X$ has a positive effect ``depends on the percentage of observations having values of Z at which the marginal effect of X is positive and significant \ldots The higher this percentage, the more inclined we would be to accept the empirical evidence as supportive.''  In this case only 4\% of the sample has incomes below \$10,000.  The support for the conflict theory, even conceding statistical significance, is minimal.  We include a discussion of this point in footnote 3 on page 7.
...
\item \textbf{Expands the Conclusion.} Presenting a close replication had the happy consequence of freeing space below the RaP word limit for us to incorporate Reviewer 1's suggestion that we expand the paper's conclusion to include a discussion of the implications of our findings.  
